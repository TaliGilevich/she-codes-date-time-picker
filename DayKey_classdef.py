...
class DayKey(Button):
    ...
    def action_report_selected_day(self, event):
        ...
        TBD following effects: deselecting previously selected day; making data of the currently selected day available to the 'upper' classes (MonthDispatcher and/or Calendar)
        ...
        if self.is_off_month:
            if selected_day < 14:  # i.e. this date belongs to the succeeding month
                print('\tbelongs to the succeeding month')  # TBD: navigation to succeeding month then setting required selection
